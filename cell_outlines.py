...
def draw_cell_coordinates_from_txt(file_path: str, image_width: int, image_height: int):
    ...
    # Save and display
    cell_outlines.save("images/cell_outlines_from_txt.png")  # Rename the image as necessary
    cell_outlines.show()
...
def draw_cell_coordinates_from_csv(file_path: str, image_width: int, image_height: int):
    ...
    predicted_value_excel = load_predicted_values("csv_data/predicted value_unicell and multicell.xlsx")
    ...
    for cell in range(1, (len(pre_cyto_outlines_df.columns) // 2) + 1):
        ...
        if pre_cyto_outlines_df.at[0, x_column] != 0 and pre_cyto_outlines_df.at[0, y_column] != 0:
            ...
            # Gradient
            color_cell(draw, color_palette, cell, cell_coordinates, predicted_value_excel)

            # Labeling
            label_cell(draw, font, cell, cell_coordinates)

    # Save and display
    cell_outlines.save("images/cell_outlines_from_csv.png")  # Rename the image as necessary
    cell_outlines.show()
...
def load_predicted_values(file_path: str):
    ...
    multicell_sheet_excel["scaled predicted cyto"] = (multicell_sheet_excel["predicted cyto"] - minimum_predicted_cyto) / (maximum_predicted_cyto - minimum_predicted_cyto)
...
def load_gradient_and_bar(cell_outlines: Image, image_width: int, image_height: int):
    ...
    color_bar_x_coordinate = (image_width - color_bar_width) // 2  # x-coordinate for placing color bar
    color_bar_y_coordinate = (image_height - color_bar_height) - 200  # y-coordinate for placing color bar
    color_bar_position = (color_bar_x_coordinate, color_bar_y_coordinate)  # top-left corner of color bar
    ...
    plt.savefig("/tmp/color_bar.png", bbox_inches="tight", transparent=True, pad_inches=0)  # saving color bar as PNG
    plt.close()
    # Loading the color bar as PIL
    color_bar_image = Image.open("/tmp/color_bar.png")
    # Pasting the color bar onto the existing image
    cell_outlines.paste(color_bar_image.resize((color_bar_width, color_bar_height)), color_bar_position)
...
def color_cell(draw: ImageDraw, color_palette: list, cell: int, cell_coordinates: list, predicted_value_excel: str):
    ...
    for index, row in predicted_value_excel.iterrows():
        if cell == row[" order"]:
            ...
            interpolated_color_index = int(row["scaled predicted cyto"] * (len(color_palette) - 1))
...
def main():
    ...
    draw_cell_coordinates_from_txt("txt_data/nucleus_20X_zstackEDF - Cropped 1_cp_outlines.txt", 5000, 5000)
